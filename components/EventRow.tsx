...
import { formatDateRange, formatShortDate, formatTimeRange, formatWeekday } from '../utils/format'
      ...
      <div className="flex items-stretch">
        <div className="w-[68px] shrink-0 pr-3 pt-5 text-right sm:w-[84px]">
          {scheduled ? (
            <>
              ...
              <span className="mt-0.5 block text-[11px] leading-tight text-slate-500">{event.end ? time.secondary : 'End time unknown'}</span>
              {showDate || event.endDate ? <span className="mt-1 block text-[11px] font-medium leading-tight text-slate-400">{event.endDate ? formatDateRange(event.date, event.endDate) : `${formatWeekday(event.date)} ${formatShortDate(event.date)}`}</span> : null}
            </>
        ...
        <div className="relative flex-1 border-l border-slate-200 pl-5">
          ...
          <div className="flex items-start gap-1 py-5">
            <button type="button" onClick={(e) => onOpen(event.id, e.currentTarget)} className="group -m-2 flex min-h-[44px] flex-1 items-start gap-2 rounded-xl p-2 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2" aria-label={`Open details for ${event.title}`}>
