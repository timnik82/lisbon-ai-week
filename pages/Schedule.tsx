...
function compareByStart(a: ScheduleEvent, b: ScheduleEvent) {
...
export function groupByDate(events: ScheduleEvent[], dateFilter: string) {
  const map = new Map<string, ScheduleEvent[]>()
  events.forEach((event) => {
    // When a specific day is selected, a multi-day event belongs under that day
    // rather than under its own start date. 'all' keeps start-date/tbd grouping.
    const key = dateFilter === 'all' ? event.date ?? 'tbd' : dateFilter === 'tbd' ? 'tbd' : dateFilter
  ...
  }).map(([key, list]) => ({ key, list: list.slice().sort(compareByStart) }))
}
